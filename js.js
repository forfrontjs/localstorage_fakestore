            ...
            arrCart.forEach((item, index) => {
                if (item.id === +el.dataset.id) {
                    if (item.counter <= 1) {
                        ...
                        let btnsCart = document.querySelector(`.btn__cart[data-id="${item.id}"]`)
                        if (btnsCart) {
                            btnsCart.classList.remove('active')
                            btnsCart.textContent = "В корзину"
                        }
                    }
    ...
    .then((data) => {
        data.forEach((el) => {
            ...
            <div class="card">
                ...
                <button class="btn__cart" data-id="${el.id}">${sameId ? 'Убрать из корзины': 'В корзину'}</button>
            </div>
            `
            renderCart()
        })
